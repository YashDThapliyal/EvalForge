...
def test_full_oracle_validation_episode_is_reconstructable(tmp_path: Path) -> None:
    scenario = build_manual_scenario("lost_confirmation", 0)
    result = run_episode(scenario, OracleAgent(scenario.oracle_plan), artifact_dir=tmp_path)
...
def test_episode_persists_live_provider_identity_and_usage(tmp_path: Path) -> None:
    ...
    agent = OracleAgent(scenario.oracle_plan)
    ...
    result = run_episode(scenario, agent, artifact_dir=tmp_path)
